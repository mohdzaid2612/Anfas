...
import {getBackgroundColor} from '../../utils/colorHelper';
...
const AppointmentHistoryHolder = props => {
  ...
  const renderAppointmentTile = ({item, index}) => {
    const {patient, date, package: selectedPackage} = item;
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View
            ...
            <EmptyAppointment width={350} height={350} />
          </View>
        }
      />
      {showDatePicker && (
        <View>
          {Platform.OS === 'ios' ? (
            <Text style={styles.okText} onPress={() => setDatePicker(false)}>
              Ok
            </Text>
          ) : null}
          <DateTimePicker
